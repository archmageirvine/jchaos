 ...
 */
public class SpaceMaker extends FreeCastable {
    ...
    if (world != null && casterCell != null) {
      ...
      do {
        ...
        while (!(ring = world.getCells(centre, ++radius, radius, false)).isEmpty()) {
          casterCell.notify(new RingCellEffectEvent(centre, radius - 1, new Color(0, 64 + Random.nextInt(192), 0)));
          // Within the ring, test if we can move its creations outwards
          content.clear();
          for (final Cell c : ring) {
            if (c.peek() != null && !wasMoved.contains(c.peek())) {
