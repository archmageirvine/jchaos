 ...
 */
public class Quench extends FreeCastable {
  @Override
  ...
  public void cast(final World world, final Caster caster, final Cell casterCell) {
    if (world != null) {
      ...
      for (int i = 0; i < world.size(); ++i) {
        ...
        if (c.peek() instanceof Fire) { // Balefire is a subclass of Fire
          affected.add(c);
        }
      }
      if (!affected.isEmpty()) {
        // At least one fire was found.  Call reinstate on each fire, to make sure
        ...
        // Award points only if at least one fire was destroyed.
